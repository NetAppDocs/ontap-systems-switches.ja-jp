...
summary: Broadcom BES-53248クラスタスイッチでSNMPv3を設定し、イーサネットスイッチヘルスモニター（CSHM）との安全な通信を有効にします。 
---
= Broadcom BES-53248クラスタスイッチのSNMPv3を設定する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチでSNMPv3を設定し、イーサネットスイッチヘルスモニター（CSHM）との安全で認証された通信を有効にします。SNMPv3は、ONTAPのヘルスモニタリングのためにスイッチのステータスを暗号化してポーリングします。
