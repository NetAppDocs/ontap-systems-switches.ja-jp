...
---
= リファレンス構成ファイル（RCF）とライセンスファイルのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
EFOS 3.12.0.1以降では、BES-53248クラスタスイッチの設定後にリファレンス構成ファイル（RCF）とライセンスファイルをインストールできます。


CAUTION: SallポートはRCFのインストール時に設定されますが、設定したポートをアクティブ化するにはライセンスをインストールする必要があります。
...
* ページからアクセスできる最新のRCF。 https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom クラスタスイッチ"^]
...
. FTP、HTTP、TFTP、SFTP、またはSCPのいずれかの転送プロトコルを使用して、スイッチcs2のブートフラッシュにRCFをコピーします。
+
