...
summary: 初期設定を完了し、必要なライセンスを適用した後、Broadcom BES-53248クラスタスイッチにリファレンス構成ファイル（RCF）をインストールします。 
---
= Broadcom BES-53248クラスタスイッチへのRCFのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初期設定を完了し、必要なライセンスを適用した後、Broadcom BES-53248クラスタスイッチにリファレンス構成ファイル（RCF）をインストールしてください。RCFは、クラスタ運用におけるポート割り当て、VLAN、およびネットワーク設定を定義します。


CAUTION: EFOS 3.12以降の場合は、link:efos-install-rcf-license-file.html["リファレンス構成ファイル（RCF）とライセンスファイルをインストールします"]のインストール手順に従ってください。
...
* 現在の RCF ファイルは、link:https://mysupport.netapp.com/site/products/all/details/broadcom-cluster-switches/downloads-tab["Broadcom クラスタスイッチ"^]ページから入手できます。
...
* RCFを工場出荷時の設定状態からインストールする際に必要な、スイッチへのコンソール接続。ナレッジベースの記事link:https://kb.netapp.com/onprem/Switches/Broadcom/How_to_clear_configuration_on_a_Broadcom_interconnect_switch_while_retaining_remote_connectivity["リモート接続を維持しながらBroadcom相互接続スイッチの設定をクリアする方法"^]を使用して事前に設定をクリアした場合、この要件は任意です。


.推奨ドキュメント
サポートされているONTAPおよびRCFバージョンについては、スイッチ互換性表を参照してください。link:https://mysupport.netapp.com/site/info/broadcom-cluster-switch["EFOSソフトウェアのダウンロード"^]を参照してください。RCFのコマンド構文とEFOSの各バージョンにおけるコマンド構文の間には、コマンドの依存関係が存在する可能性があることに注意してください。
...
* この手順の例では、4つのノードを使用します。これらのノードは2つの10GbEクラスタ インターコネクト ポート `e0a`と `e0b`を使用します。link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]を参照して、プラットフォーム上の正しいクラスタポートを確認してください。
...
. 次の点に注意してください。
+
.. スイッチに追加のポートライセンスがインストールされている場合は、追加のライセンスポートを設定するためにRCFファイルを変更する必要があります。詳細については、link:configure-licenses.html#activate-newly-licensed-ports["新しくライセンスされたポートをアクティブ化する"]を参照してください。
...
.次の手順
RCFをインストールしたら、 link:configure-ssh.html["SSHを有効にする"]。
