...
summary: Broadcom BES-53248クラスタスイッチの初期設定を実行し、EFOSソフトウェアのインストールとクラスタ展開の準備を行います。 
---
= Broadcom BES-53248クラスタスイッチの設定
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチの初期設定を実行し、EFOSソフトウェアのインストールとクラスタ展開の準備を行います。
...
* 各ノードのクラスタ ポートの名前は `e0a`および `e0b`です。プラットフォームでサポートされている実際のクラスタ ポートについては、link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]を参照してください。
...
.次の手順
スイッチの設定が完了したら、 link:configure-efos-software.html["EFOSソフトウェアをインストールする"]。
