...
summary: Broadcom BES-53248クラスタスイッチを導入する前に、クラスタポートの割り当て、ポートグループの速度制限、FEC設定など、ケーブル配線と構成に関する制約事項を理解してください。 
---
= Broadcom BES-53248クラスタスイッチのケーブル配線と構成に関する考慮事項の確認
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチを導入する前に、ケーブル配線と構成に関する制約事項を理解してください。重要な検討事項としては、クラスタポートの割り当て、ポートグループの速度制約、および25G銅線および光ファイバー接続における前方誤り訂正（FEC）設定などが挙げられます。
...
.次の手順
ケーブル配線と構成の要件を満たしたら、スイッチにlink:configure-efos-software.html["EFOSソフトウェアをインストールする"]をインストールします。
