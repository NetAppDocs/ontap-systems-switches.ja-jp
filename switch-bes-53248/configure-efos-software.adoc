...
summary: Broadcom BES-53248クラスタスイッチにEthernet Fabric OS（EFOS）をインストールして、クラスタ運用に備えます。 
---
= Broadcom BES-53248クラスタスイッチへのEFOSソフトウェアのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチにEthernet Fabric OS（EFOS）をインストールして、クラスタ運用に備えます。スイッチの設定や参照設定ファイル（RCF）のインストールを行う前に、EFOSをインストールする必要があります。
...
----
====
. インストールを完了します。スイッチを再構成するには、次の 4 つの手順に従います。
+
...
----
====
. インストールを完了します。スイッチは設定が適用されずに再起動し、工場出荷時のデフォルトにリセットされます。スイッチを再構成するには、次の 5 つの手順に従います。
+
...
.次の手順
EFOSソフトウェアをインストールしたら、以下のことができます：

* link:configure-licenses.html["ライセンスをインストールする"]。
* link:efos-install-rcf-license-file.html["EFOS 3.12.0.1以降用のRCFファイルとライセンスファイルをインストールする"]お使いのシステム構成に該当する場合。
