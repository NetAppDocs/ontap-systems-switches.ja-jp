...
summary: EFOSソフトウェアのアップグレード後、Broadcom BES-53248クラスタスイッチの参照構成ファイル（RCF）をアップグレードします。 
---
= Broadcom BES-53248クラスタスイッチのRCFをアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
EFOSソフトウェアのアップグレードと必要なライセンスの適用が完了したら、Broadcom BES-53248クラスタスイッチ上の参照構成ファイル（RCF）をアップグレードしてください。
...
* RCFを工場出荷時の設定状態からインストールする際に必要な、スイッチへのコンソール接続。
...
* BES-53248スイッチのアップグレードおよびダウングレード手順に関する完全なドキュメントについては、link:https://www.broadcom.com/support/bes-switch["Broadcom"^]サイトで入手可能な適切なソフトウェアおよびアップグレードガイドを参照してください。
...
ポート設定には、スイッチ CLI と ONTAP CLI（以下の手順で示すとおり）を使用します。まず最初のスイッチ（cs2）でポートを設定し、次に2番目のスイッチ（cs1）に対して同じ手順を繰り返します。
...
CAUTION: RCFを更新または新規適用する際は、スイッチの設定を消去し、基本設定を行う必要があります。スイッチの設定を消去するには、シリアルコンソールを使用してスイッチに接続する必要があります。
...
. 次の点に注意してください。
+
.. スイッチに追加のポートライセンスがインストールされている場合は、追加のライセンスポートを設定するためにRCFファイルを変更する必要があります。詳細については、link:configure-licenses.html#activate-newly-licensed-ports["新しくライセンスされたポートをアクティブ化する"]を参照してください。ただし、RCF 1.12以降にアップグレードすると、すべてのインターフェイスが事前に設定されるため、変更は不要になります。
...
--
. 以下の転送プロトコルのいずれかを使用して、RCFをスイッチcs2にコピーします：HTTP、HTTPS、FTP、SFTP、またはSCP。
+
...
--
====
. [[step20]]スイッチcs1でステップ1から19を繰り返します。
...
----
+
詳細については、link:https://docs.netapp.com/us-en/ontap/networking/revert_a_lif_to_its_home_port.html["ホーム ポートへのLIFのリバート"]を参照してください。
...
.次の手順
RCFをアップグレードしたら、link:configure-ssh.html["ログ収集のためにSSHを有効にする"]。
