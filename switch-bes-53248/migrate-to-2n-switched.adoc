...
summary: Broadcom BES-53248クラスタスイッチを使用して、2ノード構成のスイッチレスクラスタをスイッチドクラスタ環境に移行し、2ノードを超える拡張を可能にします。 
---
= Broadcom BES-53248クラスタスイッチを使用して、2ノードのスイッチドクラスタに移行する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチを使用して、既存の2ノードのスイッチレスクラスタをスイッチドクラスタ環境に移行します。この無停止移行により、クラスタを2ノード以上に拡張できます。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]には、お使いのプラットフォームにおける実際のクラスタ ポートに関する最新情報が含まれています。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]には、ケーブル配線に関する詳細情報が記載されています。
