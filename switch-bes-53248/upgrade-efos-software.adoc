...
summary: Broadcom BES-53248クラスタスイッチのEthernet Fabric OS（EFOS）ソフトウェアをアップグレードして、ONTAPとの互換性を維持します。 
---
= Broadcom BES-53248クラスタスイッチのEFOSソフトウェアをアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Broadcom BES-53248クラスタスイッチのEthernet Fabric OS（EFOS）ソフトウェアをアップグレードして、ONTAPとの互換性を維持し、最新のスイッチ機能と修正プログラムにアクセスできます。
...
.開始する前に
* お使いのクラスタスイッチに該当するBroadcom EFOSソフトウェアをlink:https://www.broadcom.com/support/bes-switch["Broadcomイーサネット スイッチのサポート"^]サイトからダウンロードしてください。
...
IMPORTANT: BES-53248クラスタスイッチをEFOS 3.3.x.xまたは3.4.x.xからEFOS 3.7.0.4または3.8.0.2にアップグレードした後、Inter-Switch Links（ISL）とポートチャネルが*Down*状態としてマークされることに注意してください。これは想定される動作であり、LIFの自動復元に問題が発生していない限り、アップグレードを続行しても問題ありません。詳細については、ナレッジベースの記事link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Systems/Fabric%2C_Interconnect_and_Management_Switches/BES-53248_Cluster_Switch_NDU_failed_upgrade_to_EFOS_3.7.0.4_and_later["BES-53248 クラスタスイッチNDUがEFOS 3.7.0.4以降へのアップグレードに失敗しました"^]を参照してください。
...
NOTE: 同時にRCFもアップグレードする場合は、スイッチcs1上のEFOSをアップグレードする前に、スイッチcs2上のRCFをアップグレードする手順に従ってください。詳細については、link:upgrade-rcf.html["RCFバージョンをアップグレードする"]を参照してください。

. [[ステップ12]] スイッチcs1でステップ5から11を繰り返します。
