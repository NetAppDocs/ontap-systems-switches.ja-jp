...
その link:https://hwu.netapp.com/Home/Index["_NetApp ハードウェア ユニバース_"^] ONTAP の互換性、サポートされている EFOS ファームウェア、および BES-53248 クラスタ スイッチへのケーブル接続に関する情報が含まれています。見る link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^] スイッチのインストール要件の詳細については、こちらをご覧ください。
