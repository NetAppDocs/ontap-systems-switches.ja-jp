...
summary: NVIDIA SN2100クラスタスイッチを工場出荷時のデフォルト設定にリセットするには、現在のスイッチ構成を消去します。 
---
= NVIDIA SN2100クラスタースイッチを工場出荷時のデフォルト設定にリセットする
:allow-uri-read: 


[role="lead"]
NVIDIA SN2100クラスタスイッチを工場出荷時のデフォルト設定にリセットするには、現在のスイッチ構成を消去し、スイッチを元の設定に戻します。
...
NOTE: Cumulus Linux のインストールに関する詳細については、link:configure-software-overview-sn2100-cluster.html["NVIDIA SN2100スイッチのソフトウェアインストールワークフロー"]を参照してください。
...
詳細については、NVIDIA link:https://docs.nvidia.com/networking-ethernet-software/cumulus-linux-511/Installation-Management/Factory-Reset/["工場出荷時設定へのリセット"^] のドキュメントを参照してください。

--
====
.次の手順
スイッチをリセットしたら、link:configure-overview-sn2100-cluster.html["再構成する"]必要に応じて変更します。
