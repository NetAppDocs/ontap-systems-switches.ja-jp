...
summary: NVIDIA SN2100クラスタスイッチに参照構成ファイル（RCF）スクリプトをインストールまたはアップグレードします。 
---
= NVIDIA SN2100クラスタスイッチへのRCFスクリプトのインストールまたはアップグレード
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100クラスタスイッチに参照構成ファイル（RCF）スクリプトをインストールまたはアップグレードして、ポート割り当てとネットワーク設定を構成します。

.開始する前に
RCF スクリプトをインストールまたはアップグレードする前に、スイッチで次のものが使用可能であることを確認してください。

* Cumulus Linuxがインストールされています。サポートされているバージョンについては、link:https://hwu.netapp.com/Switch/Index["Hardware Universe"^]を参照してください。
* IP アドレス、サブネット マスク、およびデフォルト ゲートウェイは DHCP 経由で定義されるか、手動で設定されます。



NOTE: ログ収集専用に使用するユーザーを RCF に指定する必要があります (管理者ユーザーに加えて)。
...
| クラスタ | 4x10GbEブレークアウト用に構成されたポートのうち、1つのポートは4x25GbEブレークアウト用に構成され、他のポートは40/100GbE用に構成されています。共有クラスタ/HAポートを使用するノードのポートにおける、共有クラスタ/HAトラフィックをサポートします。ナレッジベースの記事にあるプラットフォーム表を参照してくださいlink:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_AFF_ASA_and_FAS_platforms_use_shared_Cluster_and_HA_Ethernet_ports["共有クラスタおよび HA イーサネット ポートを使用するAFF、 ASA、およびFASプラットフォームは何ですか?"^]。すべてのポートは、専用のクラスタポートとしても使用できます。 
...
|===
.現在のRCFスクリプトのバージョン
クラスタおよびストレージ アプリケーションに使用できる RCF スクリプトは 2 つあります。 RCFをダウンロードするには https://mysupport.netapp.com/site/info/nvidia-cluster-switch["NVIDIA SN2100 ソフトウェアのダウンロード"^]ページ。それぞれの手順は同じです。

* クラスター: *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP*
* ストレージ: *MSN2100-RCF-v1._x_-Storage*


.例について
次の手順例は、クラスタ スイッチ用の RCF スクリプトをダウンロードして適用する方法を示しています。

サンプルコマンド出力では、スイッチ管理 IP アドレス 10.233.204.71、ネットマスク 255.255.254.0、デフォルトゲートウェイ 10.233.204.1 を使用しています。
...
====
. SN2100 スイッチで使用可能なインターフェースを表示します。
+
[listing, subs="+quotes"]
----
...
. RCF Python スクリプト *MSN2100-RCF-v1._x_-Cluster-HA-Breakout-LLDP* を適用します。
+
[listing, subs="+quotes"]
----
...
----
+
RCF スクリプトは、上記の例にリストされている手順を完了します。

+

NOTE: 上記の手順3 *MOTDファイルの更新*では、コマンド `cat /etc/motd`実行されます。これにより、RCF ファイル名、RCF バージョン、使用するポート、および RCF バナー内のその他の重要な情報を確認できます。

+

NOTE: 修正できないRCF Pythonスクリプトの問題については、 https://mysupport.netapp.com/["NetAppサポート"^]援助をお願いします。

. スイッチの設定に、以前に行ったカスタマイズをすべて再適用してください。必要な追加変更の詳細については、link:cabling-considerations-sn2100-cluster.html["ケーブル配線と構成に関するガイドラインを確認します。"]を参照してください。
. 再起動後に構成を確認します。
+
[listing, subs="+quotes"]
----
...
e3b       Cluster      Cluster          up   9000  auto/10000 healthy  false
----
.. クラスタからスイッチの健全性を確認します（LIFのホームがe0dでないため、スイッチsw2は表示されない可能性があります）。
+
[listing, subs="+quotes"]
----
...
NOTE: 上記の手順3 *MOTDファイルの更新*では、コマンド `cat /etc/issue.net`実行されます。これにより、RCF ファイル名、RCF バージョン、使用するポート、および RCF バナー内のその他の重要な情報を確認できます。
...
NOTE: リストされている例外はパフォーマンスに影響を与えないので、無視しても問題ありません。
...
NOTE: 上記のステップ 3 *MOTD ファイルの更新* では、コマンド `cat /etc/issue.net が実行されます。これにより、RCF ファイル名、RCF バージョン、使用するポート、および RCF バナー内のその他の重要な情報を確認できます。
...
====
.次の手順
RCFをインストールしたら、 link:setup-install-cshm-file.html["CSHMファイルをインストールする"]。
