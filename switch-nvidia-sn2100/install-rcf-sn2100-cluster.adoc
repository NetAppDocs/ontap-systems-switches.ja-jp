...
* Cumulus Linuxがインストールされています。を参照してください https://["Hardware Universe"^] を参照してください。
* IPアドレス、サブネットマスク、およびデフォルトゲートウェイは、DHCPを使用して定義するか、手動で設定します。


.現在のRCFスクリプトバージョン
クラスタアプリケーションとストレージアプリケーションには、2つのRCFスクリプトを使用できます。からRCFをダウンロードします https://["こちらをご覧ください"^]。各の手順 は同じです。
...
NOTE: 修正できないRCF Pythonスクリプトの問題については、にお問い合わせください https://["ネットアップサポート"^] を参照してください。
...
====
.次の手順
link:install-snmpv3-sn2100-cluster.html["スイッチログ収集を設定します"]。
