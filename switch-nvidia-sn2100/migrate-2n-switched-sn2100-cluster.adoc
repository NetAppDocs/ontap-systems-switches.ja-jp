...
summary: 既存の 2 ノード スイッチレス クラスタを NVIDIA SN2100 クラスタスイッチを使用してスイッチクラスタ環境に移行します。 
---
...
[role="lead"]
NVIDIA SN2100 クラスタスイッチを使用して、既存の 2 ノードスイッチレスクラスタをスイッチクラスタ環境に移行します。この無停止移行により、クラスタを 2 ノード以上に拡張することができます。
...
IMPORTANT: 注意事項および詳細については、link:cabling-considerations-sn2100-cluster.html["ケーブル配線と構成に関するガイドラインを確認します。"]を参照してください。link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- スイッチ"^]には、ケーブル配線に関する詳細情報も含まれています。
