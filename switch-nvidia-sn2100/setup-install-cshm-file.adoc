...
summary: NVIDIA SN2100クラスタスイッチのイーサネットスイッチヘルスモニター（CSHM）構成ファイルをインストールして、ONTAPでスイッチの健全性監視を有効にします。 
---
= NVIDIA SN2100クラスタスイッチ用イーサネットスイッチヘルスモニター構成ファイルのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
これらの手順は、 NVIDIA X190006-PEおよびX190006-PIスイッチが正しく検出されない場合に適用されます。これは、実行することで確認できます。 `system switch ethernet show`モデルに *OTHER* が表示されているかどうかを確認します。  NVIDIAスイッチのモデルを識別するには、次のコマンドで部品番号を見つけます。 `nv show platform hardware` NVIDIA CL 5.8以前または `nv show platform`それ以降のバージョンの場合。
