...
* Cumulus Linux（CL）OSバージョン。SN2100 Cumulusソフトウェアをダウンロードするには、にログインします https://["NVIDIAエンタープライズサポートポータル"^]。Cumulusソフトウェアにアクセスするには、ログイン資格情報が必要です。最新の互換性情報については、を参照してください https://["NVIDIAイーサネットスイッチ"^] 情報ページ。
