...
summary: 古い Cisco クラスタースイッチから NVIDIA SN2100 クラスタースイッチへ、クラスターの動作を中断することなく移行します。 
---
= Cisco クラスタスイッチから NVIDIA SN2100 クラスタスイッチへの移行
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco クラスタースイッチから NVIDIA SN2100 クラスタースイッチへ、クラスターの動作を中断することなく移行します。
...
古いCiscoクラスタスイッチをNVIDIA SN2100クラスタスイッチに交換する際には、特定の構成情報、ポート接続、およびケーブル配線の要件を把握しておく必要があります。link:configure-overview-sn2100-cluster.html["NVIDIA SN2100スイッチのインストールと構成の概要"]を参照してください。
...
NOTE: AFF A800またはAFF C800システムでe0aおよびe1aクラスタポートのポート速度を変更する場合、速度変換後に不正なパケットが受信されることがあります。link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1570339["バグ1570339"^]およびナレッジベースの記事link:https://kb.netapp.com/onprem/ontap/hardware/CRC_errors_on_T6_ports_after_converting_from_40GbE_to_100GbE["40GbEから100GbEへの変換後のT6ポートのCRCエラー"^]を参照してください。
