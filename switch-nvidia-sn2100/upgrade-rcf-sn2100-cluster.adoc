...
summary: 新しいバージョンが利用可能な場合は、NVIDIA SN2100クラスタスイッチの参照構成ファイル（RCF）スクリプトをアップグレードします。 
---
= NVIDIA SN2100クラスタスイッチのRCFスクリプトをアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいバージョンが利用可能な場合は、NVIDIA SN2100クラスタスイッチの参照構成ファイル（RCF）スクリプトをアップグレードします。

.開始する前に
RCF スクリプトをアップグレードする前に、スイッチで次のものが使用可能であることを確認してください。
...
NOTE: それでも `scp`この例では が使用されていますが、お好みのファイル転送方法を使用できます。
...
NOTE: 上記の手順3 *MOTDファイルの更新*では、コマンド `cat /etc/issue`実行されます。これにより、RCF ファイル名、RCF バージョン、使用するポート、および RCF バナー内のその他の重要な情報を確認できます。
...
NOTE: その間 `scp`この例では が使用されていますが、お好みのファイル転送方法を使用できます。
