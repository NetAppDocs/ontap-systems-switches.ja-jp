...
summary: RCFスクリプトをインストールする前に、スイッチで次のコンポーネントが使用可能であることを確認してください 
---
= RCFスクリプトをインストールします
:allow-uri-read: 
:icons: font
...
* Cumulus Linux 4.4.2がインストールされている。
...
NOTE: 次の手順 の例は、クラスタ用のRCFスクリプトをダウンロードして適用する方法を示しています。
...
----
. RCFスクリプト* MSN2100-RCF v1.8 -Cluster *を適用します。
+
[listing, subs="+quotes"]
----
...
----
+
RCFスクリプトで実行する手順は次のとおりです。

+
.. バナーのMOTDを更新します
.. apt-get for OSのアップデートを無効にします
.. ブレークアウトインターフェイスとブレークアウト以外のインターフェイスを定義します
.. インターフェイスおよびSNMPを設定します
.. CDPを無効にします
.. LLDPの設定を変更します
.. RoCE構成を追加します
.. HAおよびクラスタRDMAのRoCE設定を変更します
.. スイッチをリブートします
...
5                 5  DWRR 43%
----
. インターフェイス内のトランシーバの情報を確認します。net show interface Pluggable
+
[listing, subs="+quotes"]
----
...
----
. 各ノードが各スイッチに接続されていることを確認します。net show lldp
+
[listing, subs="+quotes"]
----
