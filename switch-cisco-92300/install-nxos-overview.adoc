...
summary: NX-OSソフトウェアと参照構成ファイル（RCF）をインストールする前に、Cisco Nexus 92300YC クラスタスイッチを準備します。 
---
= Cisco Nexus 92300YC クラスタスイッチへの NX-OS ソフトウェアと RCF のインストール準備
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NX-OS ソフトウェアと参照構成ファイル（RCF）をインストールする前に、Cisco Nexus 92300YC クラスタスイッチを準備します。スイッチが ONTAP クラスタで動作するためには、NX-OS と RCF を事前にインストールする必要があります。

.開始する前に
以下のものがあることを確認してください。

* 完全に機能するクラスター (ログにエラーや同様の問題がない)。
* 適切なソフトウェアとアップグレードガイドは、 https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 シリーズ スイッチ"^]から入手できます。
...
----
. 各クラスタ インターコネクト スイッチについて、各ノードで設定されているクラスタ インターコネクト インターフェースの数を表示します： `network device-discovery show -protocol cdp`
+
.例を表示
[%collapsible]
====
...
----
====
.. LIFに関する情報を表示します。 `network interface show -vserver Cluster`
+
.例を表示
[%collapsible]
====
...
.次の手順
NX-OS ソフトウェアと RCF をインストールする準備ができたら、link:install-nxos-software.html["NX-OSソフトウェアをインストールする"]できます。
