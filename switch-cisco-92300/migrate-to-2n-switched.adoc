...
summary: 既存の 2 ノードスイッチレスクラスタを、Cisco Nexus 92300YC クラスタスイッチを使用してスイッチクラスタ環境に移行します。 
---
= Cisco Nexus 92300YC クラスタスイッチを使用した2ノードスイッチドクラスタへの移行
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
既存の 2 ノードスイッチレスクラスタを、Cisco Nexus 92300YC クラスタスイッチを使用してスイッチクラスタ環境に移行します。この無停止移行により、クラスタを 2 ノード以上に拡張することができます。

使用する手順は、各コントローラに専用のクラスタネットワークポートが2つあるか、各コントローラにクラスタポートが1つしかないかによって異なります。記載されている手順は、光ポートまたはツインアックスポートを使用するすべてのノードで機能しますが、ノードがクラスタネットワークポートとしてオンボードの10Gb BASE-T RJ45ポートを使用している場合は、このスイッチではサポートされません。

ほとんどのシステムでは、各コントローラに2つの専用クラスタネットワーク ポートが必要です。


NOTE: 移行が完了したら、92300YCクラスタスイッチのクラスタスイッチヘルスモニタ（CSHM）をサポートするために必要な構成ファイルをインストールする必要がある場合があります。link:../switch-cshm/cshm-overview.html["スイッチヘルスモニタリング（CSHM）"]を参照してください。
...
* ノードはONTAP 9.6以降を実行しています。
* すべてのクラスター ポートは *up* 状態です。
* すべてのクラスタ論理インターフェイス (LIF) は *up* 状態であり、ホーム ポート上にあります。


Cisco Nexus 92300YC スイッチの設定：

* 両方のスイッチには管理ネットワーク接続があります。
* クラスター スイッチへのコンソール アクセスがあります。
* Nexus 92300YCのノード間スイッチ接続およびスイッチ間接続には、ツインアックスケーブルまたは光ファイバーケーブルを使用します。
+
link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- スイッチ"^]には、ケーブル配線に関する詳細情報が記載されています。

* インタースイッチリンク（ISL）ケーブルは、両方の92300YCスイッチのポート1/65と1/66に接続されています。
* 92300YCスイッチ2台の初期カスタマイズが完了しました。次の条件が満たされています：
+
** 92300YCスイッチは最新バージョンのソフトウェアを実行しています
** 参照設定ファイル（RCF）がスイッチに適用されます。SMTP、SNMP、SSH などのサイトカスタマイズは、新しいスイッチで設定されます。
...
+
link:https://hwu.netapp.com["Hardware Universe"^]には、お使いのプラットフォームにおける実際のクラスタポートに関する最新情報が含まれています。
...
----
====
. 2台の92300YCスイッチcs1とcs2間のISLおよびISL上の物理ポートが、ポート1/65と1/66で稼働していることを確認します。
+
...
----
+ 次の例は、スイッチ cs2 上の ISL ポートがアップしていることを示しています：
...
----
====
. すべてのクラスタ LIF が起動し、動作しており、 `Is Home`に対して true と表示されることを確認してください：
+
`network interface show -vserver Cluster`
...
--
====
. [[step8]] 自動ケース作成を抑制した場合は、AutoSupport メッセージを呼び出して再度有効にしてください：
+
