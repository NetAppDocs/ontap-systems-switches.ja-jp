...
summary: 初期設定後に、Cisco Nexus 92300YC クラスタスイッチに参照構成ファイル（RCF）をインストールします。 
---
= Cisco Nexus 92300YC クラスタスイッチへの RCF のインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初期設定後、Cisco Nexus 92300YC クラスタスイッチに参照構成ファイル（RCF）をインストールします。RCF は、クラスタ運用に必要なポート割り当てとネットワーク設定を定義します。
...
+
link:https://hwu.netapp.com/Home/Index["Hardware Universe"^]で、プラットフォームでサポートされている実際のクラスタポートを参照してください。スイッチの設置要件の詳細については、link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^]を参照してください。
...
* この手順の例では、2つのノードを使用します。これらのノードは2つの10GbEクラスタ インターコネクト ポート `e0a`および `e0b`を使用します。link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe"^]を参照して、プラットフォーム上の正しいクラスタポートを確認してください。
...
* サポートされている ONTAP 、 NX-OS 、および RCF バージョンについては、 https://support.netapp.com/NOW/download/software/cm_switches/["Cisco イーサネットスイッチ"^]ページのスイッチ互換性表を参照しておく必要があります。
...
. pingコマンドを使用して、RCFをホストしているサーバーへの接続を確認します。
+
...
----
. RCFファイルをNexus 92300YCスイッチにコピーします。
+
[listing, subs="+quotes"]
----
...
Copy complete.
----
. ONTAP 9.6P8以降では、以下のコマンドを使用して、スイッチ関連のログファイルを収集するためのイーサネットスイッチヘルスモニターログ収集機能を有効にします： `system cluster-switch log setup-password`および `system cluster-switch log enable-collection`
+
[listing, subs="+quotes"]
----
