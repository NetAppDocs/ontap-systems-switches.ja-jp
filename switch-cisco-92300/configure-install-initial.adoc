...
summary: Cisco Nexus 92300YC クラスタスイッチをクラスタ運用に向けて準備するために設定します。 
---
= Cisco Nexus 92300YC クラスタスイッチの設定
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 92300YC クラスタスイッチを初期設定コマンドを使用して設定し、クラスタ運用に向けて準備します。
...
. Nexus 92300YC スイッチは、初回起動時または実行中の設定を消去した後に再起動すると、起動サイクルを繰り返します。電源投入時の自動プロビジョニングを中止するには、*yes* と入力してこのサイクルを中断してください。
+
システム管理者アカウントの設定画面が表示されます。
...
----
. 基本システム設定ダイアログに入るには、*yes* と入力してください。
+
.例を表示
[%collapsible]
====
...
.次の手順
スイッチを設定したら、link:install-nxos-overview.html["NX-OSソフトウェアとRCFのインストールの準備"]できます。
