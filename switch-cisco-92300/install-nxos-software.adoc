...
summary: Cisco Nexus 92300YC クラスタスイッチにNX-OSソフトウェアをインストールして、クラスタ運用に備えます。 
---
= Cisco Nexus 92300YC クラスタスイッチに NX-OS ソフトウェアをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 92300YC クラスタスイッチにNX-OSソフトウェアをインストールして、クラスタ運用に備えます。スイッチの設定やRCFのインストールを行うには、まずNX-OSをインストールする必要があります。
...
* 該当するNetApp Cisco NX-OSソフトウェア（スイッチ用）は、NetApp Support Siteから入手できます。 http://mysupport.netapp.com/["mysupport.netapp.com"]
...
* Nexus 92300YC スイッチの名前は `cs1`および `cs2`です。
* この手順で使用される例では、2番目のスイッチである_*cs2*._でアップグレードを開始します。
...
.次の手順
NX-OS ソフトウェアをインストールしたら、link:install-the-rcf-file.html["RCFをインストールする"]できます。
