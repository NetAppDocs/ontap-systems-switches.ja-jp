...
summary: 参照構成ファイル（RCF）を Cisco Nexus 92300YC クラスタスイッチに初期設定後、または新しいバージョンが利用可能になった際にインストールまたはアップグレードします。 
---
= Cisco Nexus 92300YC クラスタスイッチへの RCF のインストールまたはアップグレード
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
参照構成ファイル（RCF）を Cisco Nexus 92300YC クラスタスイッチに初期設定後にインストールするか、新しいバージョンが利用可能になったときに RCF バージョンをアップグレードしてください。
...
* クラスタ LIF 名は `node1_clus1`、 `node1_clus2`、 `node2_clus1`、および `node2_clus2`です。
* その `cluster1::*>`プロンプトはクラスターの名前を示します。


[NOTE]
====
* この手順では、ONTAP コマンドと https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 シリーズ スイッチ"^]の両方を使用する必要があります。特に指定がない限り、ONTAP コマンドが使用されます。
...
+
.. クラスタポートがすべて正常な状態で稼働していることを確認します：
...
----
====
.. すべてのクラスタインターフェイス（LIF）がホームポートにあることを確認してください：
...
----
====
.. クラスタが両方のクラスタ インターコネクト スイッチの情報を表示していることを確認します。
...
----
. クラスタポートがクラスタスイッチcs1でホストされているポートに移行されていることを確認してください。数秒かかる場合があります。
`network interface show -vserver Cluster`
+
.例を表示
[%collapsible]
...
CAUTION: RCFを更新または新規適用する際は、スイッチの設定を消去し、基本設定を行う必要があります。スイッチを再設定するには、スイッチのシリアルコンソールポートに接続する必要があります。
...
. FTP、TFTP、SFTP、SCPのいずれかの転送プロトコルを使用して、スイッチcs2のブートフラッシュにRCFをコピーします。Ciscoコマンドの詳細については、link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 シリーズ スイッチ"^]の適切なガイドを参照してください。
+
...
+
Ciscoコマンドの詳細については、link:https://www.cisco.com/c/en/us/support/switches/nexus-9000-series-switches/series.html#InstallandUpgrade["Cisco Nexus 9000 シリーズ スイッチ"^]の適切なガイドを参照してください。

+
この例では、スイッチ cs2 に RCF ファイル `Nexus_92300YC_RCF_v1.0.2.txt`がインストールされています：
...
. スイッチの設定に、以前に行ったカスタマイズをすべて再適用してください。必要な追加変更の詳細については、link:cabling-considerations-92300.html["ケーブル配線と構成の考慮事項を確認する"]を参照してください。
...
----
. クラスタ上のクラスタポートの状態を確認します。
+
.. クラスタ内のすべてのノードで、e0dポートが稼働中で正常であることを確認してください。
`network port show -ipspace Cluster`
+
.例を表示
[%collapsible]
...
----
. クラスタ LIF がスイッチ cs2 でホストされているポートに移行されていることを確認してください。数秒かかる場合があります。 `network interface show -vserver Cluster`
+
.例を表示
[%collapsible]
====
...
----
. スイッチ cs1 を再起動してください。これは、クラスタ LIF をホームポートに戻すための操作です。スイッチの再起動中は、ノード上で報告される「クラスタポートダウン」イベントは無視して構いません。
+
[listing, subs="+quotes"]
----
cs1# *reload*
...
.
----
. cs1とcs2間のISLが正常に機能していることを確認します。
...
----
====
. クラスタ LIF がホームポートにリバートされていることを確認します：
`network interface show -vserver Cluster`
+
.例を表示
[%collapsible]
