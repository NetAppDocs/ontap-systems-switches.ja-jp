...
* のスイッチ互換性の表を参照する必要があります http://["NetApp CN1601 / CN1610 スイッチ"^] サポートされている FastPath 、 RCF 、および ONTAP のバージョンについては、ページを参照してください。
+
...
NOTE: RCF バージョン 1.2 では、セキュリティ上の理由から、 Telnet のサポートが明示的に無効になっています。RCF 1.2 のインストール時の接続の問題を回避するには、 Secure Shell （ SSH ）が有効になっていることを確認する必要があります。。 https://["『 NetApp ® CN1610 Switch Administrator's Guide 』"^] SSH の詳細については、を参照してください。
