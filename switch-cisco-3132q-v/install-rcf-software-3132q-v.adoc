...
summary: 初期設定後に、Cisco Nexus 3132Q-V クラスタスイッチに参照構成ファイル（RCF）をインストールします。 
---
= Cisco Nexus 3132Q-V クラスタスイッチへの RCF のインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初期設定後に Cisco Nexus 3132Q-V クラスタスイッチに参照構成ファイル（RCF）をインストールします。RCF は、クラスタ運用に必要なポート割り当てとネットワーク設定を定義します。
...
+
詳細については、link:https://kb.netapp.com/on-prem/Switches/Cisco-KBs/How_to_clear_configuration_on_a_Cisco_interconnect_switch_while_retaining_remote_connectivity["リモート接続を維持しながらCiscoインターコネクトスイッチの設定をクリアする方法"^]を参照してください。
...
セットアップ手順については、link:https://docs.netapp.com/us-en/ontap/task_configure_ontap.html["System Managerを使用した新しいクラスタでのONTAPの設定"]を参照してください。
