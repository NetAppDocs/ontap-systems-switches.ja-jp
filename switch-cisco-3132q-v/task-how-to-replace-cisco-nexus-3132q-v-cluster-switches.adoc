...
* クラスタスイッチ CL2 を C2 （<<step1_replace3132q,手順 1.>> -<<step21_replace3132q,21>>)
+
...
.手順
. [[step1_replace313313q]] このクラスタで AutoSupport が有効になっている場合は、 AutoSupport メッセージを呼び出してケースの自動作成を抑制します。
+
...
----
. [[step21_replace313313q]] 置き換えられた Nexus 3132Q-V スイッチが自動的に削除されていない場合は削除します。
+
