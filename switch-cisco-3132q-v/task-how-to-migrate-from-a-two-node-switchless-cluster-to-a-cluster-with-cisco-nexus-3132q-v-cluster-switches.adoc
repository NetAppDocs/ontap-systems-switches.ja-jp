...
+
** - 両方のスイッチで完全に機能する冗長な Nexus 3132 クラスタインフラストラクチャ。
+
...
[NOTE]
====
この手順 の例では、 2 つのノードを使用し、それぞれ 40/100GbE クラスタインターコネクトポート e4A と e4e を使用しています。プラットフォームのクラスタポートの詳細については、 Hardware Universe を参照してください。
...
* 最初のクラスタポートは C1 （手順 1~20 ）に移動しました。
* 2 つ目のクラスタポートは C2 に移動されました（手順 21 ～ 32 ）。
* 2 ノードスイッチレスクラスタオプションを無効にします（手順 33 ~ 35 ）。
...
----
. switchless -cluster オプションがディセーブルになっていることを確認します。
+
[listing]
----
