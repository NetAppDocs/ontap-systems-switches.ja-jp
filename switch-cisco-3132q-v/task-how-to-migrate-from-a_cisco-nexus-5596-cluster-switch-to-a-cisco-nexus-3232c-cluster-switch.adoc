...
- この手順 の例では、 Nexus 5596 スイッチを Nexus 3132Q-V スイッチに交換します。以下の手順（変更あり）を使用して、他の古い Cisco スイッチを交換できます。手順 では、スイッチとノードで次の命名法を使用します。
...
[NOTE]
====
この手順 の例では 4 つのノードを使用しています。 2 つのノードは、 e0a 、 e0b 、 e0c 、 e0d という 4 つの 10GbE クラスタインターコネクトポートを使用します。他の 2 つのノードは、 40 / 100GbE のクラスタインターコネクトポートを 2 つ使用します。 e4a 、 e4eHardware Universe には、プラットフォームの実際のクラスタポートの一覧を表示します。
