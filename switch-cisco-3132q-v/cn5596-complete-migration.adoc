...
[role="lead"]
Nexus 5596 スイッチから Nexus 3132Q-V スイッチへの移行を完了するには、次の手順を実行します。
...
[%collapsible]
====
次の例は、3132Q-V スイッチ C1 上の n1 および n2 のすべてのクラスタ相互接続ポートが起動されていることを示しています。
...
[%collapsible]
====
次の例では、新しい3132Q-VスイッチC1のすべてのノードのすべてのクラスタ相互接続ポートが `up`:
...
. 交換した Nexus 5596 が自動的に削除されない場合は削除します。
+
...
[%collapsible]
====
次の例は、Nexus 5596 を削除する方法を示しています。
