...
summary: クラスタネットワーク内の欠陥のある Cisco Nexus 3132Q-V スイッチの交換は、無停止手順（NDU）であり、特定のタスクのシーケンスを実行する必要があります。 
---
= Cisco Nexus 3132Q-V クラスタスイッチの交換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
クラスタネットワーク内の障害のある Cisco Nexus 3132Q-V スイッチを交換するには、この手順に従ってください。この交換手順は無停止手順（NDO）です。
...
.スイッチの要件
link:switch-requirements.html["Cisco Nexus 3132Q-V クラスタスイッチの交換要件"]を確認します。

.開始する前に
* 既存のクラスタおよびネットワーク構成は以下のとおりです：
+
** Nexus 3132Q-V クラスタ インフラストラクチャは冗長構成となっており、両方のスイッチで完全に機能します。
+
link:https://mysupport.netapp.com/site/products/all/details/cisco-cluster-storage-switch/downloads-tab["Ciscoイーサネット スイッチ"^]お使いのスイッチに対応した最新のRCFおよびNX-OSバージョンをご用意しています。

** すべてのクラスタポートが `up`状態です。
** 両方のスイッチに管理接続機能が備わっています。
** すべてのクラスタ論理インターフェイス（LIF）は `up`状態にあり、移行されました。


* Nexus 3132Q-V交換用スイッチについては、以下の点を確認してください。
+
** 交換スイッチ上の管理ネットワーク接続は機能しています。
** 交換用スイッチへのコンソール アクセスが確立されています。
** 目的のRCFおよびNX-OSオペレーティングシステムイメージスイッチがスイッチにロードされます。
...
この手順では、2台目のNexus 3132Q-VクラスタスイッチCL2を新しい3132Q-VスイッチC2に交換します。

.例について
この手順の例で使用するスイッチとノードの名前は次のとおりです。

* n1_clus1は、ノードn1のクラスタスイッチC1に接続された最初のクラスタ論理インターフェース（LIF）です。
* n1_clus2 は、ノード n1 のクラスタスイッチ CL2 または C2 に接続された最初のクラスタ LIF です。
* n1_clus3は、ノードn1用のクラスタスイッチC2に接続された2番目のLIFです。
...
* ノードは n1 、 n2 、 n3 、 n4 です。- この手順の例では 4 つのノードを使用します。2 つのノードは 4 つの 10GB クラスタ インターコネクト ポート（ e0a 、 e0b 、 e0c 、 e0d ）を使用します。他の2つのノードは2つの40 GBクラスタ インターコネクト ポート（e4aとe4e）を使用します。プラットフォームの実際のクラスタ ポートについては、 link:https://hwu.netapp.com/["Hardware Universe"^] を参照してください。


.タスク概要
この手順では、次のシナリオについて説明します。

* このクラスタは、2台のNexus 3132Q-Vクラスタスイッチ（CL1とCL2）に接続された4つのノードで構成されています。
* クラスタスイッチ CL2 は C2 に交換されます。
+
** 各ノードにおいて、CL2に接続されているクラスタLIFは、CL1に接続されているクラスタポートに移行されます。
** CL2のすべてのポートからケーブルを外し、交換用スイッチC2の同じポートにケーブルを再接続します。
** 各ノードにおいて、移行されたクラスタLIFが元に戻されます。
...
. 要件に応じて、新しい Nexus 3132Q-V スイッチに適切な RCF とイメージがインストールされていることを確認し、必要なサイトのカスタマイズを行ってください。
+
この時点で交換用スイッチを準備する必要があります。RCFとイメージをアップグレードする必要がある場合は、以下の手順に従ってください。

+
.. NetApp Support Site で、link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch/["Ciscoイーサネット スイッチ"^] を参照してください。
...
. スイッチC2に接続されているクラスタポートに関連付けられているLIFを移行します。
+
...
[%collapsible]
====
この例は、LIFマイグレーションがすべてのノードで実行されることを示しています。
...
[%collapsible]
====
この例は、指定されたポートがすべてのノードでシャットダウンされる様子を示しています。
...
[%collapsible]
====
この例では、スイッチ CL1 の ISL ポート 1/31 と 1/32 がシャットダウンされる様子を示します。
...
. Nexus 3132Q-VスイッチCL2に接続されているすべてのケーブルを取り外し、すべてのノードで交換用スイッチC2に再接続します。
. CL2のポートe1/31とe1/32からISLケーブルを取り外し、交換用スイッチC2の同じポートに再接続します。
...
+
ポートEth1/31とEth1/32は `(P)`を示す必要があります。これは、ISLポートがポートチャネル内でアクティブになっていることを意味します。
...
----
====
. C2でISLが稼働していることを確認します。
+
`show port-channel summary`

+
ポートEth1/31とEth1/32は `(P)`を示している必要があります。これは、両方のISLポートがポートチャネル内で稼働していることを意味します。
...
----
====
. すべてのノードで、Nexus 3132Q-V スイッチ C2 に接続されているすべてのクラスタ インターコネクト ポートを起動します：
+
`network port modify`
...
----
====
. すべてのノードで、移行したすべてのクラスタ インターコネクト LIF を元に戻します：
+
...
[%collapsible]
====
この例は、 `Current Port`列にリストされているポートの、 `Is Home`列におけるステータスが `true`であるため、すべてのLIFが正常にリバートされたことを示しています。 `Is Home`列の値が `false`である場合、LIFはリバートされていません。
...
----
====
. 交換したNexus 3132Q-Vスイッチが自動的に取り外されていない場合は、取り外してください。
+
