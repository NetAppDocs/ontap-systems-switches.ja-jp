...
summary: NX-OS ソフトウェアおよびリファレンス構成ファイル（RCF）をインストールする前に、Cisco Nexus 3132Q-V クラスタスイッチを準備します。 
---
= Cisco Nexus 3132Q-V クラスタスイッチへの NX-OS ソフトウェアと RCF のインストール準備
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NX-OSソフトウェアと参照構成ファイル（RCF）をインストールする前に、Cisco Nexus 3132Q-V クラスタスイッチを準備します。スイッチが ONTAP クラスタで動作するためには、NX-OS と RCF を事前にインストールする必要があります。
...
--
====
. [[step6]] `auto-revert`コマンドがすべてのクラスタ LIF で有効になっていることを確認します：
+
...
.次の手順
NX-OSソフトウェアとRCFのインストール準備が完了したら、link:install-nx-os-software-3132q-v.html["NX-OSソフトウェアをインストールする"]。
