...
summary: 新しいバージョンが利用可能な場合は、Cisco Nexus 3132Q-V クラスタスイッチの参照構成ファイル（RCF）をアップグレードしてください。 
---
= Cisco Nexus 3132Q-V クラスタスイッチの RCF をアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいバージョンの RCF が利用可能になった場合に、Cisco Nexus 3132Q-V クラスタスイッチの参照構成ファイル（RCF）をアップグレードします。
...
----
+
このコマンドを実行した後は、自動復元機能が無効になっていることを確認してください。
...
NOTE: 表示されるポート数は、クラスタ内のノード数によって異なります。

. クラスタポートがクラスタスイッチcs1でホストされているポートにフェイルオーバーしたことを確認してください。数秒かかる場合があります。
+
`network interface show -vserver Cluster`
...
. 現在実行中の設定ファイルと使用中の RCF ファイルの間にあるカスタム追加内容を記録してください。
+
[NOTE]
====
必ず以下を設定してください。
...
NOTE: アップグレードまたは新しい RCF を適用する場合は、スイッチの設定を消去し、基本設定を実行する必要があります。
...
+
Ciscoコマンドの詳細については、 https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 Series NX-OS コマンドリファレンス"^]の適切なガイドを参照してください。
...
NOTE: RCFのアップグレード後に10GbEポートをオンラインにする方法については、ナレッジベースの記事link:https://kb.netapp.com/onprem%2FSwitches%2FCisco%2F10GbE_ports_on_Cisco_3132Q_cluster_switch_do_not_come_online["Cisco 3132Q クラスタスイッチの 10GbE ポートがオンラインにならない"^]を参照してください。



. RCF バージョンとスイッチ設定が正しいことを確認したら、 `running-config`ファイルを `startup-config`ファイルにコピーしてください。
+
Ciscoコマンドの詳細については、link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 Series NX-OS コマンドリファレンス"^]の適切なガイドを参照してください。.例を表示
...
----
====
. スイッチ cs2 を再起動します。スイッチの再起動中にノードで報告される「クラスタポートダウン」イベントと、エラー `% Invalid command at '^' marker`出力はどちらも無視できます。
+
[listing, subs="+quotes"]
----
cs2# *reload*
This command will reboot the system. (y/n)?  [n] *y*
----
. スイッチの設定に、以前に行ったカスタマイズをすべて再適用してください。必要な追加変更の詳細については、link:cabling-considerations-3132q-v.html["ケーブル配線と構成の考慮事項を確認する"]を参照してください。
. クラスタ ポートの健全性を確認します。
+
