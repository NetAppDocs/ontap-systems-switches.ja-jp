...
summary: Cisco Nexus 3132Q-V クラスタスイッチにNX-OSソフトウェアをインストールして、クラスタ運用に向けて準備します。 
---
= Cisco Nexus 3132Q-V クラスタスイッチへの NX-OS ソフトウェアのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 3132Q-V クラスタスイッチにNX-OSソフトウェアをインストールして、クラスタ運用に向けて準備します。
...
.推奨ドキュメント
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Cisco イーサネットスイッチ"^]。サポートされているONTAPおよびNX-OSのバージョンについては、スイッチ互換性テーブルを参照してください。
* https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 シリーズスイッチ"^]。Cisco スイッチのアップグレードおよびダウングレード手順の完全なドキュメントについては、Cisco の Web サイトで入手可能な適切なソフトウェアおよびアップグレードガイドを参照してください。
...
link:prepare-install-cisco-nexus-3132q.html["NX-OSソフトウェアとReference Configuration Fileのインストールの準備"]の手順を必ず完了してから、以下の手順に従ってください。
...
. 以下の転送プロトコル（FTP、TFTP、SFTP、SCP）のいずれかを使用して、NX-OSソフトウェアをNexus 3132Q-Vスイッチにコピーします。Ciscoコマンドの詳細については、link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 Series NX-OS コマンドリファレンス"^]の適切なガイドを参照してください。
+
.例を表示
[%collapsible]
====
...
.次の手順
NX-OS ソフトウェアをインストールしたら、link:install-upgrade-rcf-overview-3132q-v.html["参照構成ファイル (RCF) をインストールまたはアップグレードする"]できます。
