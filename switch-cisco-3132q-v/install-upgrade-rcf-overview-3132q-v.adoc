...
summary: 初期設定後、または新しいバージョンが利用可能になった時点で、Cisco Nexus 3132Q-V クラスタスイッチに RCF をインストールまたはアップグレードします。 
---
= Cisco Nexus 3132Q-V クラスタスイッチへの RCF のインストールまたはアップグレードについて
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
参照構成ファイル（RCF）を Cisco Nexus 3132Q-V クラスタスイッチに初期設定後にインストールするか、新しいバージョンが利用可能になったときに RCF バージョンをアップグレードしてください。
...
.利用可能なRCF構成
以下の表は、さまざまな構成で使用可能なRCFについて説明しています。ご使用の構成に適したRCFを選択してください。
...
|===
.利用可能なRCF
次の表に、3132Q-Vスイッチで使用可能なRCFを示します。ご使用の構成に適したRCFバージョンを選択してください。詳細については、link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Ciscoイーサネット スイッチ"^]を参照してください。
...
.次の手順
RCF のインストール手順または RCF のアップグレード手順を確認したら、必要に応じてlink:install-rcf-software-3132q-v.html["RCFをインストールする"]またはlink:upgrade-rcf-3132q-v.html["RCFをアップグレードする"]を実行します。
