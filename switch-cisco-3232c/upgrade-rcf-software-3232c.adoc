...
summary: 新しいバージョンが利用可能な場合は、Cisco Nexus 3232C クラスタスイッチの参照構成ファイル（RCF）をアップグレードしてください。 
---
= Cisco Nexus 3232C クラスタスイッチの RCF をアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいバージョンの RCF が利用可能になった場合に、Cisco Nexus 3232C クラスタスイッチの参照構成ファイル （RCF） をアップグレードします。
...
.ONTAP 9.9.1以降
--
 `network interface check cluster-connectivity`コマンドを使用して、クラスタ接続のアクセシビリティチェックを開始し、詳細を表示できます：
...
--
すべての ONTAP リリースで、次の
`cluster ping-cluster -node <name>`コマンドを使用して接続を確認することもできます：
