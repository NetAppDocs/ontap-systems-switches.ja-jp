...
summary: NX-OSソフトウェアおよびリファレンス構成ファイル（RCF）をインストールする前に、Cisco Nexus 3232C クラスタスイッチを準備します。 
---
= Cisco Nexus 3232C クラスタスイッチへの NX-OS ソフトウェアと RCF のインストール準備
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NX-OSソフトウェアと参照構成ファイル（RCF）をインストールする前に、Cisco Nexus 3232Cクラスタスイッチを準備してください。スイッチがONTAPクラスタで動作するためには、NX-OSとRCFをインストールする必要があります。

.例について
この手順の例では、2つのノードを使用します。これらのノードは2つの10GbEクラスタ インターコネクト ポート `e0a`および `e0b`を使用します。

link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe"^]を参照して、プラットフォーム上の正しいクラスタポートを確認してください。スイッチの設置要件の詳細については、link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^]を参照してください。
...
* ノード名は `cluster1-01`および `cluster1-02`です。
...
--
====
. [[step6]]すべてのクラスタ LIF で `auto-revert`コマンドが有効になっていることを確認します：
...
.次の手順
NX-OS ソフトウェアと RCF をインストールする準備ができたら、link:install-nx-os-software-3232c.html["NX-OSソフトウェアをインストールする"]できます。
