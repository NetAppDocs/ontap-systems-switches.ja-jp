...
[NOTE]
====
この手順 の例では 4 つのノードを使用しています。 2 つのノードは、 e0a 、 e0b 、 e0c 、 e0d という 4 つの 10GbE クラスタインターコネクトポートを使用します。他の 2 つのノードは、 e4A 、 e4e の 2 つの 40GbE クラスタ・インターコネクト・ポートを使用します。。 link:https://hwu.netapp.com/["_ Hardware Universe _"^] に、プラットフォームの実際のクラスタポートを示します。
