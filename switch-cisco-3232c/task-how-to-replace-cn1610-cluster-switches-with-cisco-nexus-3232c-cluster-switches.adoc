...
* I.クラスタスイッチ CL2 を C2 に交換します（手順 1~22 ）。
* IIクラスタスイッチ CL1 を C1 に交換する（手順 23-40 ）


この手順 の例では 4 つのノードを使用しています。 2 つのノードは、 e0a 、 e0b 、 e0c 、 e0d という 4 つの 10GbE クラスタインターコネクトポートを使用します。他の 2 つのノードでは、 4 つの 40GbE クラスタインターコネクトファイバケーブルを使用します： e4a と e4e 。プラットフォームのクラスタファイバケーブルに関する情報については、 Hardware Universe を参照してください。
...
4 paths up, 0 paths down (udp check)
----
. 最初の CN1610 スイッチ CL1 に関連付けられている LIF を移行します。「 network interface migrate -vserver cluster-lif_lif-name __ -source-node-node-node-name_
+
...
----
. クラスタのステータスを確認します「 network interface show -role cluster
+
...
----
. スイッチ関連のログファイルを収集するために ' クラスタ・スイッチのヘルス・モニタのログ収集機能を有効にします
+
