...
この交換用手順 の例では4ノードを使用します。2 つのノードが 4 つの 10GB クラスタインターコネクトポートを使用します： e0a 、 e0b 、 e0c 、 e0d他の 2 つのノードは、それぞれ 4 GB のクラスタ・インターコネクト・ポートを 2 つ使用します。を参照してください link:https://hwu.netapp.com/SWITCH/INDEX.html["Hardware Universe"^] 使用するプラットフォームに適したクラスタポートを確認します。
