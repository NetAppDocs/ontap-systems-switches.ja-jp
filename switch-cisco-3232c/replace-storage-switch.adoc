...
summary: 障害のある Cisco Nexus 3232C ストレージ スイッチを交換します。この手順は無停止で実施できます。 
---
= Cisco Nexus 3232C ストレージ スイッチの交換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージ ネットワークの運用を中断することなく、障害のある Cisco Nexus 3232C ストレージスイッチを交換します。この無停止手順には、交換用スイッチの準備とストレージ ネットワーク構成の復元が含まれます。
...
この手順では、2台目のNexus 3232C ストレージ スイッチS2を新しい3232C スイッチNS2に交換します。2つのノードはnode1とnode2です。
...
----
====
. 動作中のスイッチで `show lldp neighbors`コマンドを実行して、両方のノードとすべてのシェルフが表示されていることを確認します。
+
`show lldp neighbors`
...
. [[five]]ストレージ システムのシェルフ ポートを確認します：
+
