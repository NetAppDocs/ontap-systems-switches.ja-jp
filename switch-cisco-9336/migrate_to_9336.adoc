...
... c1 と cs1 を接続する ISL ポートとポートチャネルが動作していることを確認します。 'how port-channel summary
+
...
... ONTAP リリース 9.5P16 、 9.6P12 、および 9.7P10 以降のパッチリリースでは、次のコマンドを使用して、スイッチ関連のログファイルを収集するためのイーサネットスイッチヘルスモニタログ収集機能をイネーブルにします。「 system cluster-switch log setup -password system cluster-switch log enable -collection
+
[listing]
----
