...
summary: NVIDIA SN2100 ストレージスイッチで SNMPv3 を設定して、安全なスイッチログ収集と正常性監視を有効にします。 
---
= NVIDIA SN2100 ストレージスイッチの SNMPv3 を設定する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100 ストレージスイッチで SNMPv3 を設定して、安全で認証済みのスイッチログ収集とイーサネットスイッチヘルスモニター（CSHM）を有効にします。

.タスク概要
次のコマンドは、 NVIDIA SN2100 スイッチで SNMPv3 ユーザー名を設定します。
...
----
====
. 新しい SNMPv3 ユーザーで監視するように SHM を構成します。
+
...
----
====
. SHM ポーリング期間が完了したら、新しく作成された SNMPv3 ユーザーで照会されるシリアル番号が前の手順で詳細に説明したものと同じであることを確認します。
+
