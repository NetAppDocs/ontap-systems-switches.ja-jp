...
summary: NVIDIA SN2100ストレージスイッチに参照構成ファイル（RCF）スクリプトをインストールまたはアップグレードします。 
---
= NVIDIA SN2100ストレージスイッチへのRCFスクリプトのインストールまたはアップグレード
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
NVIDIA SN2100ストレージスイッチに参照構成ファイル（RCF）スクリプトをインストールまたはアップグレードして、ポート割り当てとネットワーク設定を構成します。

.開始する前に
RCF スクリプトをインストールまたはアップグレードする前に、スイッチで次のものが使用可能であることを確認してください。

* Cumulus Linux 4.4.3 がインストールされています。
...
* クラスタリング: *MSN2100-RCF-v1._x_-Cluster*
* ストレージ: *MSN2100-RCF-v1._x_-Storage*



NOTE: 次の手順例は、クラスタ スイッチ用の RCF スクリプトをダウンロードして適用する方法を示しています。


NOTE: サンプルコマンド出力では、スイッチ管理 IP アドレス 10.233.204.71、ネットマスク 255.255.254.0、デフォルトゲートウェイ 10.233.204.1 を使用しています。
...
----
. RCF Python スクリプト *MSN2100-RCF-v1.8-Cluster* を適用します。
+
[listing, subs="+quotes"]
----
...
----
+
RCF スクリプトは上記の手順を完了します。

+

NOTE: 修正できないRCF Pythonスクリプトの問題については、 https://mysupport.netapp.com/["NetAppサポート"^]援助をお願いします。

. 以前のカスタマイズをスイッチ構成に再適用します。参照link:cabling-considerations-sn2100-storage.html["ケーブル配線と構成の考慮事項を確認する"]今後必要な変更の詳細については、以下を参照してください。
. 再起動後に構成を確認します。
+
`net show interface all`
...
.次の手順
RCFをインストールまたはアップグレードした後は、link:setup-install-cshm-file.html["CSHMファイルをインストールする"] 。
