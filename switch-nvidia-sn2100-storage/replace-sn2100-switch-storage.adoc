...
summary: 障害のある NVIDIA SN2100 ストレージ スイッチを交換します。これは無停止の手順です。 
---
= NVIDIA SN2100 ストレージスイッチの交換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ストレージ ネットワークの運用を中断することなく、障害のある NVIDIA SN2100 ストレージスイッチを交換します。この無停止手順には、交換用スイッチの準備とストレージ ネットワーク構成の復元が含まれます。

.開始する前に
NVIDIA SN2100ストレージスイッチに Cumulus ソフトウェアと RCF をインストールする前に、以下の点を確認してください：

* システムはNVIDIA SN2100 ストレージ スイッチをサポートできます。
* 該当する RCF をダウンロードしました。


link:http://hwu.netapp.com["Hardware Universe"^]には、サポートされているポートとその設定に関する詳細情報が記載されています。
