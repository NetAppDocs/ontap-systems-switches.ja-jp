...
summary: Cisco Nexus 9808 スイッチの NX-OS をアップグレードして、AFX 2K システムの新しいハードウェアまたは新しい ONTAP バージョンをサポートします。 
---
= NX-OS をアップグレードする - Cisco Nexus 9808
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9808 スイッチの NX-OS をアップグレードして、AFX 2K システムの新しいハードウェアまたは新しい ONTAP バージョンをサポートします。
...
+
** AFX 2K システム内のスイッチの場合、ONTAP 9.19.1GA 以降、および NX-OS 10.6 以降の実行がサポートされています。
+
...
最初のスイッチ（cs1）に NX-OS をインストールするには、新しいバージョンの NX-OS と EPLD をインストールして検証し、クラスタ上のすべてのポートの健全性を確認してから、2番目のスイッチ（cs2）でこれらの手順を繰り返します。

. スイッチのCLIを使用して、NX-OSとEPLDイメージをスイッチにコピーします。
+
...
----
====
. スイッチのCLIを使用して、NX-OSとEPLDイメージをインストールします。
+
...
----
====
. スイッチのCLIを使用して、新しいバージョンのEPLDが正常にロードされたことを確認します。
+
...
----
====
. スイッチcs2にNX-OSとEPLDイメージをインストールするには、同じ手順を繰り返してください。
...
.次の手順
スイッチの NX-OS をアップグレードしたら、link:configure-install-upgrade-rcf-overview-9808.html["RCFの概要をインストールまたはアップグレードする"] から始めて RCF をアップグレードします。
