...
keywords:  
summary: 直接接続ストレージを備えたスイッチレスクラスタから、新たに2台の共有スイッチを追加することで、Cisco Nexus 9336C-FX2共有スイッチに移行します。 
---
= 直接接続ストレージを備えたスイッチレスクラスタから Cisco Nexus 9336C-FX2 共有スイッチへの移行
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
直接接続ストレージを備えたスイッチレスクラスタから、新たに2台の共有スイッチを追加することで、Cisco Nexus 9336C-FX2共有スイッチに移行します。
...
ほとんどのシステムでは、各コントローラに2つの専用クラスタネットワーク ポートが必要です。詳細についてはlink:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Ciscoイーサネット スイッチ"^]を参照してください。
...
** Nexus 9336C-FX2 のノード間スイッチおよびスイッチ間接続には、Twinax ケーブルまたは光ファイバー ケーブルを使用します。
** link:https://hwu.netapp.com["Hardware Universe"^]には、ケーブル配線に関する詳細情報が記載されています。
** スイッチ間リンク (ISL) ケーブルは、両方の 9336C-FX2 スイッチのポート 1/35 と 1/36 に接続されています。
...
* この手順で使用されるクラスタポートは、AFF A400 コントローラに従って _e3a_ と _e3b_ です。link:https://hwu.netapp.com["Hardware Universe"^]には、お使いのプラットフォームの実際のクラスタポートに関する最新情報が記載されています。
...
+
link:https://hwu.netapp.com["Hardware Universe"^]には、ケーブル配線に関する詳細情報が記載されています。スイッチの設置要件の詳細については、link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^]を参照してください。
