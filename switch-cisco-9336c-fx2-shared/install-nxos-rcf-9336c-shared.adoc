...
summary: 参照構成ファイル（RCF）を Cisco Nexus 9336C-FX2 共有スイッチの初期設定後にインストールして、クラスタとストレージを組み合わせた運用のためのポート割り当てを設定します。 
---
= Cisco Nexus 9336C-FX2共有スイッチにRCFをインストールする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
初期設定後に、Cisco Nexus 9336C-FX2 共有スイッチに参照構成ファイル（RCF）をインストールします。RCF は、クラスタとストレージを組み合わせた運用におけるポート割り当てを設定します。

始める前に、以下の手順を完了してください。link:prepare-nxos-rcf-9336c-shared.html["NX-OSとRCFのインストールの準備"] 。
...
.利用可能なRCF構成
* *ClusterStorageRCF* - パーティション化されたクラスターとスイッチ上の 2 つのストレージ ゾーンをサポートします (Cluster-Storage RCF 1._xx_)。
...
.次の手順
RCFをインストールしたら、 link:../switch-cshm/config-overview.html["スイッチのヘルスモニタリングを設定する"]。
