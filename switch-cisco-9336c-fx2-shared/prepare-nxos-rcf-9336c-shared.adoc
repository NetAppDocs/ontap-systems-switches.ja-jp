...
keywords:  
summary: NX-OSソフトウェアとリファレンス構成ファイル（RCF）をインストールする前に、Cisco Nexus 9336C-FX2共有スイッチを準備します。 
---
= Cisco Nexus 9336C-FX2共有スイッチへのNX-OSソフトウェアとRCFのインストール準備
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NX-OSソフトウェアとリファレンス構成ファイル（RCF）をインストールする前に、Cisco Nexus 9336C-FX2共有スイッチを準備します。スイッチがONTAP環境で動作するためには、NX-OSとRCFをインストールする必要があります。
...
----
====
. 各クラスタ インターフェイスと各ノード ストレージ ポートおよびストレージ シェルフ ポートの管理ステータスまたは動作ステータスを確認します。
+
.. ネットワーク ポートの属性を表示します。
+
[source, cli]
...
.次の手順
NX-OSソフトウェアとRCFをインストールする準備ができたら、 link:install-nxos-software-9336c-shared.html["NX-OSソフトウェアをインストールする"]。
