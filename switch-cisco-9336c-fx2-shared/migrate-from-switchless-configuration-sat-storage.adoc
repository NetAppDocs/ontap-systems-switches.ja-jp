...
keywords:  
summary: 既存のストレージスイッチを再利用して、スイッチレス構成からスイッチ接続ストレージを備えた Cisco Nexus 9336C-FX2 共有スイッチに移行します。 
---
= スイッチレス構成からスイッチ接続ストレージを使用した Cisco Nexus 9336C-FX2 共有スイッチへの移行
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
既存のストレージスイッチを再利用して、スイッチレス構成からスイッチ接続ストレージを備えた Cisco Nexus 9336C-FX2 共有スイッチに移行します。
...
. [[step7]]共有スイッチ A 上のストレージ RCF を共有 RCF ファイルに置き換えます。詳細については、link:install-nxos-rcf-9336c-shared.html["Cisco Nexus 9336C-FX2共有スイッチにRCFをインストールする"]を参照してください。
...
. [[step13]]共有スイッチB上のストレージRCFファイルを共有RCFファイルに置き換えます。詳細については、link:install-nxos-rcf-9336c-shared.html["Cisco Nexus 9336C-FX2共有スイッチにRCFをインストールする"]を参照してください。
. HA ペア 1 に接続されているストレージ、ストレージ スイッチ B が正常であることを確認します。
+
`system health alert show -instance`
...
. [[step16]]HA ペア 1 をスイッチレスクラスタからスイッチドクラスタに変換します。共有 RCF で定義されているクラスタポート割り当てを使用します。詳細については、link:prepare-nxos-rcf-9336c-shared.html["NX-OS ソフトウェアおよびリファレンス構成ファイル（ RCF ）をインストールします"]を参照してください。
