...
keywords:  
summary: 障害のある Cisco Nexus 9336C-FX2 共有スイッチを交換します。この手順は無停止で実行できます。 
---
...
[role="lead"]
クラスタまたはストレージ ネットワークの動作を中断することなく、障害のある Cisco Nexus 9336C-FX2 共有スイッチを交換します。この無停止手順（NDU）では、交換用スイッチを準備し、共有スイッチの構成を復元します。
...
. [[step18]]棚のケーブルが正しく接続されていることを確認します。
+
