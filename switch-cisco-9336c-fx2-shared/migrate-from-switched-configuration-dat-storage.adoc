...
keywords:  
summary: 直接接続ストレージを備えたスイッチ構成から、新たに2台の共有スイッチを追加することで Cisco Nexus 9336C-FX2 共有スイッチに移行します。 
---
= 直接接続ストレージを使用したスイッチ構成から Cisco Nexus 9336C-FX2 共有スイッチへの移行
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
直接接続ストレージを備えたスイッチ構成から、新たに2台の共有スイッチを追加することで Cisco Nexus 9336C-FX2 共有スイッチに移行します。
...
この手順でサポートされている ONTAP および NX-OS のバージョンは、Cisco イーサネット スイッチのページに記載されています。を参照してください https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Ciscoイーサネットスイッチ"^]。
...
link:https://hwu.netapp.com["Hardware Universe"^]には、すべてのクラスタ スイッチでサポートされているケーブル接続に関する情報が含まれています。
