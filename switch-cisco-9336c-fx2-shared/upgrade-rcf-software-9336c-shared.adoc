...
.. NX-OS 10.2以降では、コマンドを使用し `show diff running-config`て、ブートフラッシュに保存されているRCFファイルと比較します。それ以外の場合は、サードパーティの差分ツールまたは比較ツールを使用してください。
...
+
SSHキーに関連するホストファイルのエントリの更新が必要になる場合があります。
