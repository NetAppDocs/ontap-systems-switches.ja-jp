...
summary: 新しいバージョンが利用可能になった場合に、Cisco Nexus 9336C-FX2 共有スイッチの参照構成ファイル （RCF） をアップグレードします。 
---
= Cisco Nexus 9336C-FX2共有スイッチのRCFをアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいバージョンの RCF が利用可能な場合に、Cisco Nexus 9336C-FX2 共有スイッチの参照構成ファイル（RCF）をアップグレードします。
...
----
====
. 各クラスタ ポート、ノード ストレージ ポート、およびストレージ シェルフ ポートの管理ステータスと動作ステータスを確認します。
+
...
+
.. 現在実行中の構成と使用中の RCF ファイル間のカスタム追加 (組織の SNMP 構成など) を記録します。
...
. 詳細についてはlink:cabling-considerations-9336c-shared.html["ケーブル配線と構成の考慮事項を確認する"]を参照してください。
...
====
.次の手順
RCFをアップグレードしたら、 link:../switch-cshm/config-overview.html["スイッチのヘルスモニタリングを設定する"]。
