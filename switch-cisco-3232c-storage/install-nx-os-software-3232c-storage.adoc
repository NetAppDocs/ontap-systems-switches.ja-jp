...
.開始する前に
次のものがあることを確認してください。* スイッチ設定の最新のバックアップ。* 完全に機能しているクラスタ（ログにエラーや同様の問題がない）。* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Ciscoイーサネット スイッチ ページ"^]。サポートされているONTAPおよびNX-OSのバージョンについては、スイッチ互換性テーブルを参照してください。* link:https://www.cisco.com/c/en/us/support/switches/nexus-3000-series-switches/products-installation-guides-list.html["Cisco Nexus 3000 シリーズ スイッチ"^]。Ciscoスイッチのアップグレードおよびダウングレード手順に関する完全なドキュメントについては、CiscoのWebサイトで入手可能な適切なソフトウェアおよびアップグレードガイドを参照してください。
