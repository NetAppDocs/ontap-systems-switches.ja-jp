...
summary: Cisco Nexus 3232C ストレージスイッチに NX-OS ソフトウェアをインストールまたはアップグレードします。 
---
= Cisco Nexus 3232C ストレージスイッチへの NX-OS ソフトウェアのインストールまたはアップグレード
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 3232C ストレージスイッチにNX-OSソフトウェアをインストールまたはアップグレードして、ストレージ ネットワーク運用の準備を行います。
...
* link:https://mysupport.netapp.com/site/info/cisco-ethernet-switch["Ciscoイーサネット スイッチ ページ"^]。サポートされているONTAPおよびNX-OSのバージョンについては、スイッチ互換性テーブルを参照してください。
...
----
====
. NX-OS バージョン 9.3(11) にアップグレードする場合は、EPLD `golden` イメージをアップグレードし、スイッチを再度再起動してください。それ以外の場合は、手順 12 に進んでください。
+
詳細については、 https://www.cisco.com/c/en/us/td/docs/switches/datacenter/nexus9000/sw/93x/epld-rn/nxos_n9K_epldRN_9311.html["EPLDアップグレードリリースノート、リリース9.3(11)"^]を参照してください。
...
----
====
. スイッチの再起動後、ログインして新しいバージョンの EPLD が正常にロードされたことを確認してください。
+
.例を表示
[%collapsible]
====
...
----
====
. スイッチを初めて設定する場合は、手順 15 に進んでください。スイッチをアップグレードする場合は、次の手順に進んでください。
