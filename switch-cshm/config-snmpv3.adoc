...
summary: ネットワーク スイッチで SNMPv3 を設定して、イーサネット スイッチ ヘルス モニター（CSHM）との安全な通信を有効にします。 
---
= イーサネットスイッチの状態監視のためにSNMPv3を設定する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ネットワーク スイッチで SNMPv3 を設定して、イーサネット スイッチ ヘルス モニター（CSHM）との安全で認証された通信を有効にします。SNMPv3 は、ONTAP でのヘルス監視のためにスイッチのステータスを暗号化してポーリングします。

イーサネットスイッチヘルスモニター（CSHM）は、SNMPを使用してネットワークスイッチの状態とパフォーマンスを監視します。デフォルトでは、SNMPv2cは参照設定ファイル（RCF）を通じて自動的に設定されます。SNMPv3はSNMPv2よりも安全です。なぜなら、SNMPv3は認証、暗号化、メッセージの完全性などの堅牢なセキュリティ機能を導入し、不正アクセスを防ぎ、送信中のデータの機密性と完全性を確保するためです。
...
.Ciscoスイッチ
--
Cisco Nexus スイッチで SNMPv3 ユーザー名 SNMPv3_USER を設定します：
...
.NVIDIA - CL 5.4.0
--
CL 5.4.0 を実行している NVIDIA Cumulus スイッチで SNMPv3 ユーザー名 SNMPv3_USER を設定します：
...
.NVIDIA - CL 5.11.0+
--
CLI 5.11.0 以降を実行している NVIDIA Cumulus スイッチで SNMPv3 ユーザー名 SNMPv3_USER を設定します：
