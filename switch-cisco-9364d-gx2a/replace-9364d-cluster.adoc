...
summary: AFX システムで障害のある Cisco Nexus 9364D-GX2A スイッチを、運用を中断することなく交換します。これは無停止手順です。 
---
= AFXシステムのCisco Nexus 9364D-GX2Aスイッチの交換
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX システムで障害のある Cisco Nexus 9364D-GX2A スイッチを、運用を中断することなく交換します。この無停止手順（NDU）では、交換用スイッチの準備とシステム構成の復元を行います。
...
* Nexus 9364D-GX2A 交換スイッチの場合:
+
** 交換スイッチ上の管理ネットワーク接続は機能しています。
** 交換用スイッチへのコンソール アクセスが確立されています。
** ノード接続はポート1/1～1/62です。
** ポート 1/63 および 1/64 のすべての Inter-Switch Link (ISL) ポートが無効になっています。
** 必要な参照構成ファイル (RCF) と NX-OS オペレーティング システム イメージ スイッチがスイッチにロードされます。
** スイッチの初期カスタマイズが完了しました。詳細はlink:configure-complete-initial-setup-9364d.html["スイッチの初期設定を完了する"]を参照してください。
+
...
* 既存の Nexus 9364D-GX2A スイッチの名前は cs1 と cs2 です。
* 新しい Nexus 9364D-GX2A スイッチの名前は newcs2 です。
...
. Nexus 9364D-GX2A スイッチ cs1 の ISL ポート 1/63 と 1/64 をシャットダウンします。
+
[listing, subs="+quotes"]
----
cs1# *config*
...
cs1(config)# *exit*
----
. すべてのケーブルをNexus 9364D-GX2B cs2スイッチから取り外し、Nexus 9364D-GX2A newcs2スイッチの同じポートに接続します。
. cs1スイッチとnewcs2スイッチ間でISLポート1/63と1/64を起動し、ポート チャネルの動作ステータスを確認します。
+
ポートチャネルはPo1（SU）を示し、メンバー ポートはEth1/63（P）およびEth1/64（P）を示している必要があります。

+
.例を表示
[%collapsible]
====
次に、ISLポート1/63および1/64を有効にし、スイッチcs1のポート チャネルの概要を表示する例を示します。
