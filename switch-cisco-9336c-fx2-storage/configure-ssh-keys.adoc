...
summary: SSH と SSH キーが Cisco Nexus 9336C-FX2 ストレージ スイッチで有効になっていることを確認し、CSHM およびログ収集をサポートします。 
---
= Cisco Nexus 9336C-FX2 ストレージスイッチのSSH構成を確認する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9336C-FX2 および 9336C-FX2-T ストレージスイッチで SSH と SSH キーが有効になっていることを確認し、イーサネットスイッチヘルスモニター（CSHM）およびログ収集機能をサポートします。
...
IMPORTANT: FIPSを有効にする場合は、 `ssh key ecdsa 256 force`コマンドを使用してスイッチのビット数を256に変更する必要があります。詳細については、link:https://docs.netapp.com/us-en/ontap/networking/configure_network_security_using_federal_information_processing_standards_@fips@.html#enable-fips["FIPSを使用してネットワークセキュリティを構成する"^]を参照してください。
