...
summary: 既存の 2 ノード スイッチレス クラスタを、Cisco Nexus 9336C-FX2 および 9336C-FX2-T ストレージ スイッチを使用してスイッチ クラスタ環境に移行します。 
---
= Cisco Nexus 9336C-FX2 ストレージスイッチを使用した2ノードスイッチドクラスタへの移行
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
既存の 2 ノードスイッチレスクラスタを、Cisco Nexus 9336C-FX2 および 9336C-FX2-T ストレージスイッチを使用してスイッチクラスタ環境に移行します。この無停止移行により、クラスタを 2 ノード以上に拡張することができます。
...
* 見る link:https://hwu.netapp.com/SWITCH/INDEX["Hardware Universe- スイッチ"^] ケーブル接続の詳細については、こちらをご覧ください。
* 見る link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^] スイッチのインストール要件の詳細については、こちらをご覧ください。
