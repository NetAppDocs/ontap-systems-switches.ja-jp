...
summary: 新しいバージョンが利用可能な場合に、Cisco Nexus 9336C-FX2 および 9336C-FX2-T クラスタスイッチの参照構成ファイル （RCF） をアップグレードします。 
---
= Cisco Nexus 9336C-FX2 および 9336C-FX2-T クラスタスイッチの RCF をアップグレードする
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
新しいバージョンの RCF が利用可能になった場合に、Cisco Nexus 9336C-FX2 および 9336C-FX2-T クラスタスイッチの参照構成ファイル（RCF）をアップグレードします。
...
CAUTION: ネットワーク接続の問題を回避するため、接続されているクラスタ ポートを*すべて*シャットダウンしてください。詳細については、ナレッジベースの記事link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["スイッチ OS のアップグレード中にクラスタ LIF を移行するとノードがクォーラム外になる"^]を参照してください。
...
. 以下の転送プロトコル（FTP、SFTP、SCP）のいずれかを使用して、RCFをスイッチcs1のbootflashにコピーします。
+
Ciscoコマンドの詳細については、link:https://www.cisco.com/c/en/us/support/switches/nexus-9336c-fx2-switch/model.html#CommandReferences["Cisco Nexus 9000 シリーズ NX-OS コマンド リファレンス"^]の適切なガイドを参照してください。

+
この例では、SFTPを使用してスイッチcs1のブートフラッシュにRCFをコピーしています。
...
CAUTION: RCFアップグレード後に9336C-FX2スイッチが起動しない場合は、NetAppナレッジベースの記事link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Cisco_9336C-FX2_Switch_Fails_to_Boot_After_RCF_Install_%E2%80%93_Boot_Variable_Cleared["Cisco 9336C-FX2スイッチがRCFインストール後に起動に失敗する - ブート変数がクリアされている"^]を参照して次のステップを確認してください。
