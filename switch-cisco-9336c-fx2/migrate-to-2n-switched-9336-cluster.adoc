...
summary: 既存の 2 ノード スイッチレス クラスタを、Cisco Nexus 9336C-FX2 および 9336C-FX2-T クラスタスイッチを使用してスイッチクラスタ環境に移行します。 
---
= Cisco Nexus 9336C-FX2 クラスタスイッチを使用して2ノードスイッチドクラスタに移行する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
既存の 2 ノード スイッチレス クラスタを、Cisco Nexus 9336C-FX2 および 9336C-FX2-T クラスタスイッチを使用してスイッチクラスタ環境に移行します。この無停止移行により、クラスタを 2 ノード以上に拡張することができます。
...
** サポートされているすべての ONTAP バージョンについては、link:https://hwu.netapp.com["Hardware Universe"^]を参照してください。
...
+
ケーブル配線の詳細については、link:https://hwu.netapp.com["Hardware Universe"^]を参照してください。
...
link:https://hwu.netapp.com["Hardware Universe"^]で、プラットフォームのクラスタポートに関する情報を参照してください。スイッチの設置要件の詳細については、link:https://kb.netapp.com/Support/General_Support/What_additional_information_do_I_need_to_install_my_equipment_that_is_not_in_HWU["HWU にない機器をインストールするには、どのような追加情報が必要ですか?"^]を参照してください。
...
[%collapsible]
====
次の例は、すべての LIF が node1 と node2 で起動していることを示しています：
