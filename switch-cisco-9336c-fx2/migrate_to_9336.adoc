...
. 各ノードのクラスタポートは、（ノードから見て）次のように既存のクラスタスイッチに接続されています：「 network device-discovery show -protocol cdp
+
[listing, subs="+quotes"]
----
...
----
. クラスタネットワークが完全に接続されていることを確認するには、「 cluster ping-cluster -node node-name 」コマンドを使用します
+
[listing, subs="+quotes"]
----
...
----
. cs1 のポート e1/33~34 で、 c1 と cs1 の間の一時的な ISL を設定します。
+
...
cs2(config)# *exit*
----
. c2 のポート e1/33~34 から ISL ケーブルを取り外し、 cs2 のポート e1/33~34 にケーブルを接続します。
. c2 と cs2 に接続している ISL ポートとポートチャネルが動作していることを確認します「 show port-channel summary
+
...
101   Po101(SU)   Eth      LACP      Eth1/41(P)   Eth1/42(P)   Eth1/43(P)
                                     ...
                                     Eth1/47(P)   Eth1/48(P)
----
. ノード 1 について、 c2 の e1/1 からケーブルを外し、 Nexus 9336C-FX2 でサポートされている適切なケーブルを使用して cs2 の e1/1 に接続します。
. ノード 2 について、 c2 の e1/2 からケーブルを外し、 Nexus 9336C-FX2 でサポートされている適切なケーブルを使用して cs2 の e1/2 に接続します。
. これで、各ノードのクラスタポートは、ノードから見て次のようにクラスタスイッチに接続されました。「 network device-discovery show -protocol cdp
+
[listing, subs="+quotes"]
----
cluster1::*> *network device-discovery show -protocol cdp*
...
----
. ノード 1 について、 c1 の E1/1 からケーブルを外し、 Nexus 9336C-FX2 でサポートされている適切なケーブルを使用して、 cs1 の E1/1 にケーブルを接続します。
. ノード 2 について、 c1 の e1/2 からケーブルを外し、 Nexus 9336C-FX2 でサポートされている適切なケーブルを使用して、 cs1 の e1/2 にケーブルを接続します。
. これで、各ノードのクラスタポートは、ノードから見て次のようにクラスタスイッチに接続されました。「 network device-discovery show -protocol cdp
+
[listing, subs="+quotes"]
----
...
cluster1::*>
----
. ONTAP 9.8 以降の場合は、「 system switch ethernet log setup-password 」および「 system switch ethernet log enable-collection 」の 2 つのコマンドを使用して、スイッチ関連のログファイルを収集するためのイーサネットスイッチヘルスモニタログ収集機能をイネーブルにします
+
