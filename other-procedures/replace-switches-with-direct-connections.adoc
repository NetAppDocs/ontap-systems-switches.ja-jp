...
summary: 2ノードクラスタ内のスイッチは、ノード間の直接のバックツーバック接続で交換できます。 
---
= スイッチを直接接続に交換します
:allow-uri-read: 
:icons: font
...
クラスタインターコネクトスイッチを使用する2ノードクラスタがある場合は、スイッチをノード間の直接のバックツーバック接続に交換します。これはノンストップオペレーションです。

.作業を開始する前に
* クラスタは正常に動作しており、クラスタスイッチで接続された2つのノードで構成されている必要があります。また、ノードで同じONTAP リリースが実行されている必要があります。
* 各ノードには、システム構成をサポートするための冗長なクラスタインターコネクト接続を提供するために必要な数の専用クラスタポートが必要です。たとえば、各ノードに2つの専用クラスタインターコネクトポートがあるシステムの場合、2つの冗長ポートが必要です。
...
----
+
[Enable Switchless Cluster Detection](スイッチレスクラスタ検出を有効にする)が[false]の場合は、サポートにお問い合わせください。
...
+
次の例に示すように'ポートのLinkカラムの値がUpで' Health Statusカラムの値がHealthyであることを確認します
...
+
[Discovered Device]列には、ポートが接続されているクラスタスイッチの名前を指定します。次の例は、クラスタポート「e0a」と「e0b」がクラスタスイッチ「cs1」と「cs2」に正しく接続されていることを示しています。
...
+
次の例では、node1の「e0a」がnode2の「e0a」に接続され、node2の「e0b」がnode2の「e0b」に接続されています。
...
. すべてのLIFがホームにあることを確認する。この処理には数秒かかることがあります。
+
...
link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/How_to_suppress_automatic_case_creation_during_scheduled_maintenance_windows_-_ONTAP_9["ネットアップの技術情報アーティクル 1010449 ：「 How to suppress automatic case creation during scheduled maintenance windows"^]
