...
summary: AFX システムで障害のある Cisco Nexus 9332D-GX2B スイッチを、運用を中断することなく交換します。これは無停止手順です。 
---
= AFXシステムのCisco Nexus 9332D-GX2Bスイッチを交換する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFX システムで障害のある Cisco Nexus 9332D-GX2B スイッチを、運用を中断することなく交換します。この無停止手順（NDU）では、交換用スイッチの準備とシステム構成の復元を行います。
...
** スイッチの初期カスタマイズが完了しました。詳細については、link:configure-complete-initial-setup-9332d.html["スイッチの初期設定を完了する"]を参照してください。
+
...
* メンテナンスの前後にメンテナンスAutoSupportをトリガーして、メンテナンス期間中のケース作成を無効にします。詳細については、このナレッジベース記事 https://kb.netapp.com/Support_Bulletins/Customer_Bulletins/SU92["SU92: スケジュールされたメンテナンス期間中の自動ケース作成を抑制する方法"^]を参照してください。
...
----
====
. newcs2スイッチのEth1/1からEth1/30までのすべてのポートを起動します。
+
.例を表示
[%collapsible]
====
