...
summary: Cisco Nexus 9332D-GX2B スイッチのリファレンス構成ファイル（RCF）のバージョンをアップグレードすることで、新しいハードウェア、新しい ONTAP バージョンのサポート、または AFX 1K や AFX 2K システム構成における新しい NX-OS バージョンへの対応が可能になります。 
---
= RCFバージョンのアップグレード - Cisco Nexus 9332D-GX2B
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Cisco Nexus 9332D-GX2B スイッチのリファレンス構成ファイル（RCF）のバージョンをアップグレードすることで、新しいハードウェア、新しい ONTAP バージョンのサポート、または AFX 1K や AFX 2K システム構成における新しい NX-OS バージョンへの対応が可能になります。
...
CAUTION: ネットワーク接続の問題を避けるため、接続されている*すべて*のポートを必ずシャットダウンしてください。詳細については、ナレッジベースの記事 https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/Node_out_of_quorum_when_migrating_cluster_lif_during_switch_OS_upgrade["スイッチ OS のアップグレード中にクラスタ LIF を移行するとノードがクォーラム外になる"^]を参照してください。
...
+
スイッチがRCF 10.1以降を実行している場合は、必要に応じてlink:configure-install-upgrade-rcf-overview-9332d.html#change-port-speed-configuration["ポート速度設定を変更する"]できます。
...
----
====
. ONTAP CLIを使用して、リモートクラスタインターフェイスの接続性を確認します。
+
...
.次の手順
スイッチの RCF をアップグレードしたら、link:configure-verify-ssh-9332d.html["ログ収集のためにSSHを確認する"]。
