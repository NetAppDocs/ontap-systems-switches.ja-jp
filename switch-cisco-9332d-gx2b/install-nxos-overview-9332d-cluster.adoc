...
summary: 'Cisco NX-OS ソフトウェアとリファレンス コンフィギュレーション ファイル (RCF) をCisco Nexus 9332D-GX2B クラスタ スイッチにインストールする必要があります。' 
---
...
.. *クラスターポート*
+
... 各スイッチの各ノードに設定されているクラスタ相互接続インターフェースの数を表示します。
+
[source, cli]
----
...
.次の手順
NX-OSソフトウェアとRCFをインストールする準備ができたら、 link:install-nxos-software-9332d-cluster.html["NX-OSソフトウェアをインストールまたはアップグレードする"]。
